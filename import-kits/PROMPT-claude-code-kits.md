...
Ajoute les kits scolaires dans l'application : 48 kits, 16 classes du CI à la Terminale (séries L et S), 3 gammes par classe. Tout est dans le dossier `import-kits/` à la racine du dépôt :

- `kits.json` : les 48 kits, leurs lignes, les 143 références produits qu'ils utilisent, et les séries à venir
- `SacAdo_kits_v5.xlsx` : la même chose en tableur, pour l'équipe. Ne l'utilise pas pour l'import.
...
- `complet` : toute la liste, fournitures de qualité supérieure et livres au programme.
- `confort` : le Complet, plus des livres en plus (lectures conseillées, ou La Clé des Cracks pour les classes scientifiques).
...
**Kit** : `slug` (unique), `titre`, `classe`, `cycle` (`elementaire`, `college`, `lycee`), `serie` (`L`, `S`, `STEG` ou vide), `gamme`, `ordre_gamme`, `description`, `description_si_aucune_cle_des_cracks`, `ebook_offert`, `type_source`, `statut` (`masque` par défaut), plus deux champs réservés à l'admin : `source_interne` et `manquants_connus`.
...
- Tous les kits sont importés en `statut = masque`. La publication est une action manuelle.
...
- Mention « Ebook offert » si `ebook_offert` vaut vrai. S'il existe un produit ou un mécanisme ebook dans l'application, utilise-le. Sinon, affiche seulement la mention : n'invente pas d'ebook.
...
**Séries à venir.** `kits.json` contient une liste `series_a_venir` : la série STEG (Seconde, Première, Terminale) et le lycée arabe (profils scientifique et littéraire). Là où l'utilisateur choisit sa classe et sa série, affiche ces séries avec leur `message` (« Les kits de la série STEG ne sont pas encore disponibles. »), sans prix, sans bouton d'achat et sans lien vers un kit. Aucun kit n'est créé pour elles.
...
1. Lance l'import deux fois : toujours 48 kits (16 par gamme), aucun doublon.
...
5. `prix_achat` n'apparaît dans aucune réponse publique.
6. Lance le build et les tests existants. Corrige ce que ton changement a cassé.
