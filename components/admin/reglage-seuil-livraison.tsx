...
export function ReglageSeuilLivraison({ valeur }: { valeur: number }) {
  ...
  const enregistrer = async () => {
    setSaving(true);
    setError(null);
    const res = await reglerSeuilLivraisonGratuite(Number(n));
    setSaving(false);
    if (!res.ok) {
      setError(res.error);
      return;
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-ink/10 bg-white px-3.5 py-2.5 text-sm">
      <span className="text-ink/70">Livraison gratuite à partir de</span>
      <input
        type="number"
        min={0}
        value={n}
